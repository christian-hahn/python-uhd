...
def main():
    ...
    parser.add_argument("signal_freq", help="Signal frequency in Hz.", type=float)
    parser.add_argument("--gain", help="RX gain in dB.", type=float, default=40.0)
    parser.add_argument(
        "--max-deviation", help="Maximum deviation in Hz.", type=float, default=75.0e3
    )
    ...
    min_samp_rate = float((abs(args.signal_freq - lo_freq) + signal_bw) * 2.0)
    ...
    master_clock_rate = float(samp_rate * np.floor(61.44e6 / samp_rate / 4.0) * 4.0)
    ...
    audio_num_samps = int(round(audio_samp_rate * 5.0))  # 5 second blocks
    ...
    dnconv = np.exp(
        ...
        * 2.0
        * np.pi
        * ((args.signal_freq - lo_freq) / samp_rate)
        * np.arange(num_samps)
    )

    # IF and audio low-pass filters
    if_filter = cheby1(N=8, rp=3.0, Wn=signal_bw / samp_rate, btype="low")
    audio_filter = cheby1(N=8, rp=3.0, Wn=15.0e3 / if_samp_rate, btype="low")

    # De-emphasis filter
    decay = np.exp(-1.0 / (if_samp_rate * 75e-6))
    ...
    try:
        ...
        while True:
            ...
            print(
                "avg_pwr = {:.3f} dBfs, peak_pwr = {:.3f} dBfs".format(
                    avg_pwr, peak_pwr
                )
            )
            ...
            samps = samps * (
                2.0**14 / (args.max_deviation / if_samp_rate * (2.0 * np.pi))
            )
