...
class SpecAn(object):
    ...
    def __init__(self, samp_rate, num_points, num_chans=1,
                 title=None, limits=None, freq_offset=0.):
    ...
    def update(self, samps):
        ...
        for samp, trace in zip(samps,self.traces):
            psd = 10.*np.log10(np.fft.fftshift(signal.welch(samp, self.samp_rate,
                               nperseg=self.num_points, scaling='spectrum', return_onesided=False)[1]))
            trace.set_ydata(psd)
            ymin = min(ymin, min(psd)) if ymin else min(psd)
            ymax = max(ymax, max(psd)) if ymax else max(psd)
        if not self.limits:
            yrange = ymax - ymin
            self.axes.set_ylim([ymin-yrange*0.1, ymax+yrange*0.1] if yrange else [ymin-1, ymax+1])
