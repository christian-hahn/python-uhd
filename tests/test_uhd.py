...
def is_range(obj):
    ...
    return (
        isinstance(obj, dict)
        and all(k in obj for k in keys)
        and all(isinstance(obj[k], float) for k in keys)
    )
...
class UhdTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        for chan in range(self.dut.get_rx_num_channels()):
            self.dut.set_rx_freq(self.dut.get_rx_freq_range(chan)["start"], chan)
        for chan in range(self.dut.get_tx_num_channels()):
            self.dut.set_tx_freq(self.dut.get_tx_freq_range(chan)["start"], chan)
    ...
    def helper_range(self, f_set, f_get, f_range, steps=None):
        ...
        if is_range(r):
            ...
            values = (
                np.arange(r["start"], r["stop"], r["step"])
                if steps is None
                else np.linspace(r["start"], r["stop"], steps)
            )
        ...
        for value in values:
            f_set(value)
            ret = f_get()
            if is_range(r) and not isclose(r["step"], 0.0):
    ...
    def test_normalized_rx_gain(self):
        ...
        for chan in range(self.dut.get_rx_num_channels()):
            self.helper_range(
                lambda x: self.dut.set_normalized_rx_gain(x, chan),
                lambda: self.dut.get_normalized_rx_gain(chan),
                lambda: {"start": 0.0, "step": 0.0, "stop": 1.0},
                steps=10,
            )
    ...
    def test_normalized_tx_gain(self):
        ...
        for chan in range(self.dut.get_rx_num_channels()):
            self.helper_range(
                lambda x: self.dut.set_normalized_tx_gain(x, chan),
    ...
    def test_rx_freq(self):
        ...
        for chan in range(self.dut.get_rx_num_channels()):
            self.helper_range(
                lambda x: self.dut.set_rx_freq(x, chan),
                lambda: self.dut.get_rx_freq(chan),
                lambda: self.dut.get_rx_freq_range(chan),
                steps=10,
            )
            r = self.dut.get_rx_freq_range(chan)
            target_freq = round((r["start"] + r["stop"]) / 2.0 / 1.0e9) * 1.0e9
            for policy in ["none", "auto", "manual"]:
                tune_result = self.dut.set_rx_freq(
                    {
                        "target_freq": target_freq,
                        "lo_off": 0.0,
                        "rf_freq_policy": "manual",
                        "rf_freq": target_freq,
                        "dsp_freq_policy": "manual",
                        "dsp_freq": 0.0,
                        "args": "mode_n=integer",
                    },
                    chan,
    ...
    def test_tx_freq(self):
        ...
        for chan in range(self.dut.get_tx_num_channels()):
            self.helper_range(
                lambda x: self.dut.set_tx_freq(x, chan),
            ...
            target_freq = round((r["start"] + r["stop"]) / 2.0 / 1.0e9) * 1.0e9
            for policy in ["none", "auto", "manual"]:
                tune_result = self.dut.set_tx_freq(
                    {
                        "target_freq": target_freq,
                        "lo_off": 0.0,
                        "rf_freq_policy": "manual",
    ...
    def test_rx_bandwidth(self):
        ...
        for chan in range(self.dut.get_rx_num_channels()):
            self.helper_range(
                lambda x: self.dut.set_rx_bandwidth(x, chan),
    ...
    def test_tx_bandwidth(self):
        ...
        for chan in range(self.dut.get_tx_num_channels()):
            self.helper_range(
                lambda x: self.dut.set_tx_bandwidth(x, chan),
    ...
    def test_rx_gain(self):
        ...
        for chan in range(self.dut.get_rx_num_channels()):
            self.helper_range(
                lambda x: self.dut.set_rx_gain(x, chan),
    ...
    def test_tx_gain(self):
        ...
        for chan in range(self.dut.get_tx_num_channels()):
            self.helper_range(
                lambda x: self.dut.set_tx_gain(x, chan),
    ...
    def test_rx_antenna(self):
        ...
        for chan in range(self.dut.get_rx_num_channels()):
            self.helper_range(
                lambda x: self.dut.set_rx_antenna(x, chan),
    ...
    def test_tx_antenna(self):
        ...
        for chan in range(self.dut.get_tx_num_channels()):
            self.helper_range(
                lambda x: self.dut.set_tx_antenna(x, chan),
    ...
    def test_rx_rate(self):
        ...
        for chan in range(self.dut.get_rx_num_channels()):
            ...
            self.helper_range(
                lambda x: self.dut.set_rx_rate(x, chan),
                lambda: self.dut.get_rx_rate(chan),
                lambda: [mcr / 2.0**n for n in range(5)],
            )
    ...
    def test_tx_rate(self):
        ...
        for chan in range(self.dut.get_tx_num_channels()):
            ...
            self.assertIsInstance(rates, dict)
            self.helper_range(
                lambda x: self.dut.set_tx_rate(x, chan),
    ...
    def test_mboard_getters(self):
        ...
        if hasattr(self.dut, "get_filter_names"):
            ...
            self.assertTrue(
                int(pyuhd.UHD_VERSION_ABI.split(".")[0]) >= 4,
    ...
    def test_rx_setters(self):
        ...
        for chan in range(self.dut.get_rx_num_channels()):
            ...
            self.dut.set_rx_dc_offset(0.0 + 0j, chan)
            # set_rx_iq_balance
            for value in [True, False]:
                self.dut.set_rx_iq_balance(value, chan)
            self.dut.set_rx_iq_balance(0.0 + 0j, chan)

    def test_tx_setters(self):
        """Test TX-specific setters."""
        for chan in range(self.dut.get_tx_num_channels()):
            # set_tx_dc_offset
            self.dut.set_tx_dc_offset(0.0 + 0j, chan)
            # set_tx_iq_balance
            self.dut.set_tx_iq_balance(0.0 + 0j, chan)
    ...
    def test_receive(self):
        ...
        self.dut.set_master_clock_rate(16.0e6)
        self.dut.set_rx_rate(1.0e6)
        ...
        for channels_type, channels_case in product(channels_types, channels_cases):
            ...
            for num_samps in (100, 2**12, 2**16, 2**20):
                for kwargs in kwargs_cases:
                    print(
                        "Testing RX (channels = {}, num_samps = {}, kwargs = {})".format(
                    ...
                    try:
                        if isinstance(kwargs, tuple):
                            ...
                            samps, start = self.dut.receive(
                                num_samps, channels, **kwargs
                            )
                        ...
                        self.fail(
                            "Failed to receive (channels = {}, num_samps = {}, kwargs"
    ...
    def test_transmit(self):
        ...
        for channels_type, channels_case in product(channels_types, channels_cases):
            ...
            for num_samps in (2**12, 2**16, 2**20):
                ...
                for kwargs in kwargs_cases:
                    print(
                        "Testing TX (channels = {}, num_samps = {}, kwargs = {})".format(
                            channels, num_samps, kwargs
                        )
                    )
                    try:
                        ...
                        self.fail(
                            "Failed to transmit (channels = {}, num_samps = {}, kwargs"
                            " = {}): {}".format(channels, num_samps, kwargs, str(e))
                        )
