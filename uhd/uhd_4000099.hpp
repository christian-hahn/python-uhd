...
PyObject *Uhd_get_gpio_src(Uhd *self, PyObject *args) {
    ...
    try {
        std::lock_guard<std::mutex> lg(self->dev_lock);
        if (nargs == 2)
            ret = self->dev->get_gpio_src(bank.get(), mboard.get());
        else
            ret = self->dev->get_gpio_src(bank.get());
    } catch(const uhd::exception &e) {
        return PyErr_Format(UhdError, "%s", e.what());
    }
...
PyObject *Uhd_get_gpio_src_banks(Uhd *self, PyObject *args) {
    ...
    try {
        std::lock_guard<std::mutex> lg(self->dev_lock);
        if (nargs == 1)
            ret = self->dev->get_gpio_src_banks(mboard.get());
        else
            ret = self->dev->get_gpio_src_banks();
    } catch(const uhd::exception &e) {
        return PyErr_Format(UhdError, "%s", e.what());
    }
...
PyObject *Uhd_get_gpio_srcs(Uhd *self, PyObject *args) {
    ...
    try {
        std::lock_guard<std::mutex> lg(self->dev_lock);
        if (nargs == 2)
            ret = self->dev->get_gpio_srcs(bank.get(), mboard.get());
        else
            ret = self->dev->get_gpio_srcs(bank.get());
    } catch(const uhd::exception &e) {
        return PyErr_Format(UhdError, "%s", e.what());
    }
...
"          \"0/Radio#0:HB_0\"\n"
PyObject *Uhd_get_rx_filter_names(Uhd *self, PyObject *args) {
    ...
    try {
        std::lock_guard<std::mutex> lg(self->dev_lock);
        ret = self->dev->get_rx_filter_names(chan.get());
    } catch(const uhd::exception &e) {
        return PyErr_Format(UhdError, "%s", e.what());
    }
...
"Returns:\n" \
"    dict: value\n"
PyObject *Uhd_get_rx_power_range(Uhd *self, PyObject *args) {
    ...
    try {
        std::lock_guard<std::mutex> lg(self->dev_lock);
        ret = self->dev->get_rx_power_range(chan.get());
    } catch(const uhd::exception &e) {
        return PyErr_Format(UhdError, "%s", e.what());
    }
...
"Returns:\n" \
"    float: value\n"
PyObject *Uhd_get_rx_power_reference(Uhd *self, PyObject *args) {
    ...
    try {
        std::lock_guard<std::mutex> lg(self->dev_lock);
        if (nargs == 1)
            ret = self->dev->get_rx_power_reference(chan.get());
        else
            ret = self->dev->get_rx_power_reference();
    } catch(const uhd::exception &e) {
        return PyErr_Format(UhdError, "%s", e.what());
    }
...
"    list: Filter names will follow the pattern BLOCK_ID:FILTER_NAME. For example,\n" \
"          \"0/Radio#0:HB_0\"\n"
PyObject *Uhd_get_tx_filter_names(Uhd *self, PyObject *args) {
    ...
    try {
        std::lock_guard<std::mutex> lg(self->dev_lock);
        ret = self->dev->get_tx_filter_names(chan.get());
    } catch(const uhd::exception &e) {
        return PyErr_Format(UhdError, "%s", e.what());
    }
...
"Returns:\n" \
"    dict: value\n"
PyObject *Uhd_get_tx_power_range(Uhd *self, PyObject *args) {
    ...
    try {
        std::lock_guard<std::mutex> lg(self->dev_lock);
        ret = self->dev->get_tx_power_range(chan.get());
    } catch(const uhd::exception &e) {
        return PyErr_Format(UhdError, "%s", e.what());
    }
...
"Returns:\n" \
"    float: value\n"
PyObject *Uhd_get_tx_power_reference(Uhd *self, PyObject *args) {
    ...
    try {
        std::lock_guard<std::mutex> lg(self->dev_lock);
        if (nargs == 1)
            ret = self->dev->get_tx_power_reference(chan.get());
        else
            ret = self->dev->get_tx_power_reference();
    } catch(const uhd::exception &e) {
        return PyErr_Format(UhdError, "%s", e.what());
    }
...
PyObject *Uhd_has_rx_power_reference(Uhd *self, PyObject *args) {
    ...
    try {
        std::lock_guard<std::mutex> lg(self->dev_lock);
        if (nargs == 1)
            ret = self->dev->has_rx_power_reference(chan.get());
        else
            ret = self->dev->has_rx_power_reference();
    } catch(const uhd::exception &e) {
        return PyErr_Format(UhdError, "%s", e.what());
    }
...
PyObject *Uhd_has_tx_power_reference(Uhd *self, PyObject *args) {
    ...
    try {
        std::lock_guard<std::mutex> lg(self->dev_lock);
        if (nargs == 1)
            ret = self->dev->has_tx_power_reference(chan.get());
        else
            ret = self->dev->has_tx_power_reference();
    } catch(const uhd::exception &e) {
        return PyErr_Format(UhdError, "%s", e.what());
    }
...
PyObject *Uhd_set_rx_power_reference(Uhd *self, PyObject *args) {
...
"    chan (int, optional): the channel index 0 to N-1\n"
PyObject *Uhd_set_rx_spp(Uhd *self, PyObject *args) {
...
"    mboard (int, optional): which motherboard to set the config\n"
PyObject *Uhd_set_sync_source(Uhd *self, PyObject *args) {
...
"    power_dbm (float): The reference power level in dBm\n" \
"    chan (int, optional): The channel for which this setting applies\n"
PyObject *Uhd_set_tx_power_reference(Uhd *self, PyObject *args) {
...
const std::vector<PyMethodDef> Uhd_gen_methods {
    ...
    {"get_gpio_src", (PyCFunction)Uhd_get_gpio_src, METH_VARARGS, DOC_GET_GPIO_SRC},
    {"get_gpio_src_banks", (PyCFunction)Uhd_get_gpio_src_banks, METH_VARARGS, DOC_GET_GPIO_SRC_BANKS},
    {"get_gpio_srcs", (PyCFunction)Uhd_get_gpio_srcs, METH_VARARGS, DOC_GET_GPIO_SRCS},
    ...
    {"get_rx_filter_names", (PyCFunction)Uhd_get_rx_filter_names, METH_VARARGS, DOC_GET_RX_FILTER_NAMES},
    ...
    {"get_rx_power_range", (PyCFunction)Uhd_get_rx_power_range, METH_VARARGS, DOC_GET_RX_POWER_RANGE},
    {"get_rx_power_reference", (PyCFunction)Uhd_get_rx_power_reference, METH_VARARGS, DOC_GET_RX_POWER_REFERENCE},
    ...
    {"get_tx_filter_names", (PyCFunction)Uhd_get_tx_filter_names, METH_VARARGS, DOC_GET_TX_FILTER_NAMES},
    ...
    {"get_tx_power_range", (PyCFunction)Uhd_get_tx_power_range, METH_VARARGS, DOC_GET_TX_POWER_RANGE},
    {"get_tx_power_reference", (PyCFunction)Uhd_get_tx_power_reference, METH_VARARGS, DOC_GET_TX_POWER_REFERENCE},
    ...
    {"has_rx_power_reference", (PyCFunction)Uhd_has_rx_power_reference, METH_VARARGS, DOC_HAS_RX_POWER_REFERENCE},
    {"has_tx_power_reference", (PyCFunction)Uhd_has_tx_power_reference, METH_VARARGS, DOC_HAS_TX_POWER_REFERENCE},
    ...
    {"set_rx_power_reference", (PyCFunction)Uhd_set_rx_power_reference, METH_VARARGS, DOC_SET_RX_POWER_REFERENCE},
    {"set_rx_rate", (PyCFunction)Uhd_set_rx_rate, METH_VARARGS, DOC_SET_RX_RATE},
    {"set_rx_spp", (PyCFunction)Uhd_set_rx_spp, METH_VARARGS, DOC_SET_RX_SPP},
    {"set_rx_subdev_spec", (PyCFunction)Uhd_set_rx_subdev_spec, METH_VARARGS, DOC_SET_RX_SUBDEV_SPEC},
    {"set_sync_source", (PyCFunction)Uhd_set_sync_source, METH_VARARGS, DOC_SET_SYNC_SOURCE},
    ...
    {"set_tx_power_reference", (PyCFunction)Uhd_set_tx_power_reference, METH_VARARGS, DOC_SET_TX_POWER_REFERENCE},
